...
# transform
fuel_df = duckdb.query('''select regexp_replace(price, '[a-zA-Z]', '', 'g') price, station, address, city, report_time, strptime(concat(year(current_date), ' ', report_time), '%Y %b %-d, %-H:%M %p') report_time_formatted from fuel_df''').df()
